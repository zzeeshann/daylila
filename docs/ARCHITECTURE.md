...
- [x] `<lesson-shell>` + `<lesson-beat>` Web Components — beat-by-beat paginated layout (single mode since C7, 2026-05-08; previously dual-mode behind `admin_settings.reading_mode`). `<lesson-shell>` is the step coordinator: builds a step list from `<lesson-beat>` + `[data-lesson-step]` regions in DOM order, owns `currentStep`, listens for `audio-player:requeststep` + `lesson-progress:goto`, dispatches `lesson-shell:stepchange`. Hides non-current beats + step regions via the CSS rule in `src/styles/lesson-pagination.css`, gated on `:root[data-lesson-hydrated="true"]` (set by lesson-shell on connect — the no-JS guard).
- [x] Beat CSS in standalone `src/styles/beats.css` + `src/styles/lesson-pagination.css` (both not Tailwind-processed, survive purging). Pagination CSS gates the hide-rule on the hydrated guard.
- [x] AudioPlayer web component — beat-aware `<audio-player>` that reads `audioBeats: { beatName → publicUrl }` from frontmatter and plays per-beat clips. Always coordinated (since C7): subscribes to `lesson-shell:stepchange` and loads the matching clip when the new step is a beat with audio; pauses + re-labels caption on non-audio steps. Prev/next button + clip-end auto-advance dispatch `audio-player:requeststep` for lesson-shell to handle. The `autoplayOnNextLoad` flag bridges the clip-end → stepchange round-trip so auto-advance keeps playing across the boundary. Graceful degrade: missing audio shows "coming soon"; 404 shows "unavailable".
- [x] BaseLayout + LessonLayout with breadcrumbs, date eyebrow, beat/subject meta line. LessonLayout renders `<lesson-progress>` (slim row of dots above audio player) + inline companion interactive section + finish-state footer. Each `[data-lesson-step]` region (interactive, quiz, finish) is one paginated step.
- [x] Progressive enhancement (without JS, beats show as continuous prose). The CSS hide-rule gates on `:root[data-lesson-hydrated]` (JS-set by lesson-shell on connect); without JS, the attribute never appears, the rule never matches, every beat renders. Same fallback shape as Area 5's pre-pagination layout.
