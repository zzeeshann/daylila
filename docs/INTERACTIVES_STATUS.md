...
**Phase 3, sub-task 3.2 — Admin list view at `/dashboard/admin/interactives/`.** Shipped 2026-04-26. New SSR Astro page (folder route, leaving room for per-interactive deep-dive pages later). ADMIN_EMAIL gated. Stats row (total / clean / rough / quiz / html) + soft "pieces missing quiz / html" line above the catalogue. Type filter chip bar (All / Quiz / HTML) mirrors the admin observer-feed severity-chip pattern verbatim. Per-row card: type pill (quiz/html), status badge (Clean / Rough / Pending), tier label when set, title link to public `/interactives/<slug>/`, source-piece headline link to admin per-piece deep-dive, voice score, four audit pills for the latest round (voice/structure/essence/factual with pass/fail + voice score), revision count, published date. Sort: `quality_flag='low'` first, then by `published_at DESC`. Two D1 queries (interactives + headline join, then latest-round audit fetch) stitched in app code. Regenerate cell rendered as a placeholder ("Regenerate: shipping in 3.3") so 3.3 just slots in the button without a layout shift. New nav-link "Interactives →" in admin home top-right block.

`pnpm build` clean. Auth gating verified locally (redirects to `/login/?redirect=/dashboard/admin/interactives/`). Authenticated UI verifies post-deploy. See DECISIONS 2026-04-26 "Interactives v3 Phase 3.2 — admin list view at `/dashboard/admin/interactives/`".
...
**Phase 3, sub-task 3.3 — Per-piece interactive regenerate.** New agents-worker endpoint `POST /interactive-regenerate-trigger?piece_id=<uuid>&type=<quiz|html|both>` mirroring the existing `/audio-retry?mode=fresh` shape: wipes the existing `interactives` row(s) + `interactive_audit_results` rows for the piece-type combo, clears `daily_pieces.interactive_id` if applicable, then triggers a fresh produce → audit → revise loop. Site-side proxy at `/api/agents/interactive-regenerate.ts` ADMIN_EMAIL-gated. Replaces the placeholder cell on each row of the 3.2 list view with a working button + confirm dialog warning about content/git churn (each regenerate produces a new commit on a fresh slug if the slug-collision fallback fires, or overwrites the file in place if not). Update the per-piece admin page's Retry button to also use the new endpoint when the operator wants a destructive regen vs. the existing idempotent first-run path.
