# Daylila v2 — Living Architecture
...
- [x] `<lesson-shell>` + `<lesson-beat>` Web Components — beat-by-beat paginated layout (single mode since C7, 2026-05-08; previously dual-mode behind `admin_settings.reading_mode`). `<lesson-shell>` is the step coordinator: builds a step list from `<lesson-beat>` + `[data-lesson-step]` regions in DOM order, owns `currentStep`, listens for `audio-player:requeststep` + `lesson-progress:goto`, dispatches `lesson-shell:stepchange`. Step kinds are `'beat' | 'interactive' | 'quiz'` (since C8b, 2026-05-08; the `'finish'` kind was dropped — the finish footer is no longer a paginated step). Hides non-current beats + step regions via the CSS rules in `src/styles/lesson-pagination.css`, gated on `:root[data-lesson-hydrated="true"]` (set by lesson-shell on connect — the no-JS guard). On every step change, lesson-shell sets `:root[data-lesson-on-last-step="true"]` when the active step is the final entry in the step list, deletes the attribute otherwise; the finish-footer hide-rule keys off the absence of that attribute.
- [x] Beat CSS in standalone `src/styles/beats.css` + `src/styles/lesson-pagination.css` (both not Tailwind-processed, survive purging). Pagination CSS gates the hide-rules on the hydrated guard; the finish-footer rule additionally keys off `:root[data-lesson-on-last-step]`.
- [x] AudioPlayer web component — beat-aware `<audio-player>` that reads `audioBeats: { beatName → publicUrl }` from frontmatter and plays per-beat clips. Always coordinated (since C7): subscribes to `lesson-shell:stepchange` and loads the matching clip when the new step is a beat with audio. On non-audio steps (interactive / quiz), the chrome hides via `style.display = 'none'` (since C8a, 2026-05-08; previously it relabelled the caption "Step 7 of 9 · Interactive" over a dead play button — the play button did nothing because there was no clip to play). Default-hidden synchronously at the top of `connectedCallback` so a hash-resumed non-audio step (e.g. `/piece/#interactive`) doesn't flash the audio chrome before the first stepchange runs; restored to visible at connect when the initial shell step is a beat with audio. Prev/next button + clip-end auto-advance dispatch `audio-player:requeststep` for lesson-shell to handle. The `autoplayOnNextLoad` flag bridges the clip-end → stepchange round-trip so auto-advance keeps playing across the boundary. Graceful degrade: missing audio shows "coming soon"; 404 shows "unavailable".
- [x] BaseLayout + LessonLayout with breadcrumbs, date eyebrow, beat/subject meta line. LessonLayout renders `<lesson-progress>` (slim row of dots above audio player) + inline companion interactive section + finish-state footer. Each `[data-lesson-step]` region (interactive, quiz) is one paginated step. The finish footer is NOT a paginated step (since C8b, 2026-05-08); it renders structurally below the article and the lesson-pagination CSS rule shows it only while the reader is on the last step in the list — the quiz step on a quiz piece, the interactive step on an interactive-only piece, the close beat on a no-companion piece.
- [x] Progressive enhancement (without JS, beats show as continuous prose). The CSS hide-rules gate on `:root[data-lesson-hydrated]` (JS-set by lesson-shell on connect); without JS, the attribute never appears, the rules never match, every `<lesson-beat>` + `[data-lesson-step]` region + the `[data-lesson-finish]` footer renders. Same fallback shape as Area 5's pre-pagination layout. The new `:root[data-lesson-on-last-step]` flag from C8b is also JS-set; without JS, the finish-footer rule never matches and the footer renders at the bottom of the article alongside everything else.
...
- [x] D1 database: 22 tables, 30 migrations (see `docs/SCHEMA.md`)
...
- [x] AudioProducerAgent — ElevenLabs TTS (Frederick Surrey, `eleven_multilingual_v2`, `mp3_44100_96`), saves per-beat MP3 to R2, writes `daily_piece_audio` rows. 20k-char budget cap, 3-attempt retry, request-stitching for prosodic continuity. Text normalisation via `agents/src/shared/tts-normalize.ts` (provider-agnostic): "Daylila → Zee-mish" prosody alias + Roman-numeral → spelled-word conversion (e.g. "Schedule IV and V" → "Schedule four and five") with pronoun-safe three-pass logic. Live 2026-04-18; normaliser layer added 2026-04-23.
...
- [x] `robots.txt` advertises the sitemap via `Sitemap: https://daylila.com/sitemap.xml` directive at the bottom. Disallows `/dashboard/admin/`, `/api/`, `/auth/`, `/account/`.
- [x] BaseLayout `<head>` carries `<link rel="alternate" type="application/rss+xml" title="Daylila daily pieces" href="/rss.xml" />` so Feedly / Inoreader / NetNewsWire auto-discover the feed from any page.
