...
**Phase 3, sub-task 3.4 — Cost telemetry.** Surface MTD token count for InteractiveGenerator runs on the list view (or a dedicated `/dashboard/admin/cost/` if the surface grows beyond interactives), broken cached vs uncached. Decide schema route at the start of the work: existing `observer_events` rows from `logInteractiveGeneratorMetered` already carry `tokensIn` / `tokensOut` per artefact in the JSON `context`. If those numbers + extension to other agents (Drafter, Voice/Structure/Fact auditors, Curator) is cheap, extend `observer_events` query side; if not, ship a new `agent_cost_telemetry` table with one row per Claude call. Expectation is the observer-events read path wins because the data is already there for InteractiveGenerator and a `JSON_EXTRACT` aggregate is fast at current row counts. Surface on the existing admin list view as a stats-row line ("$X.XX MTD across N runs · Y% cached"), no separate page yet.
