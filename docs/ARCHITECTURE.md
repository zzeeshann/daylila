...
- [x] FactCheckerAgent — verifies claims (single-pass: Claude with the Anthropic `web_search_20250305` server tool, today's date in user message, search-first for current-event claims). Returns per-claim `sources: [{url, title, citedText, searchQuery}]` enriched from `web_search_result_location` citation blocks; cross-references against URLs Claude names in each claim's `sources` array (drops hallucinated URLs). Exposes `searchAvailable` on result; Director logs an Observer warn when web_search returns `unavailable` so the pipeline honours the "no silent failure" principle. Replaced the 2026-04-19 DDG IA leg on 2026-04-30 (owns its prompt file).
