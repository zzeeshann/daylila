...
- **The two errors:**
  1. **`src/server.ts` Durable Object stub typing — 26 errors.** `DurableObjectNamespace<undefined>` doesn't carry the agent class methods (`triggerDailyPiece`, `retryAudio`, `analyseZitaPatternsScheduled`, etc.). The same shape this file's earlier 2026-04-19 entry covered for an older count of 25. Code works at runtime; tsc just can't resolve the methods. Cloudflare Agents SDK typing gap. Fix is either (a) cast at every `.get(stub)` call site to the right agent type, or (b) wait for an SDK release that exports a typed `DurableObjectNamespace<T>` shape. (b) is preferable — (a) bloats the file with casts that go stale on every method addition. The earlier 2026-04-19 entry is still open at line 1188 of this file; this is the same issue at a higher count after subsequent agent methods landed.
  2. **`src/audio-auditor.ts:200` arity mismatch.** `persistAuditRows` declared at line 236 takes 3 parameters (`pieceId, runId, result`); the call site at line 200 passes only 2 (`brief.pieceId, result`). Line 111 passes 3 correctly. The line-200 call is the bug. Net effect at runtime: line 200 only fires inside the retry path; if it ever fired, it would silently skip the `runId` bind on the `audio_audit_results` INSERT — i.e. the row would land but with `run_id` as the result object's serialised form (TypeScript erases the type at runtime, so `runId='[object Object]'` or similar would land). May explain a small population of audio_audit_results rows with garbage run_ids in prod — worth a one-line query to check before fixing.
- **Hypothesis:** (1) is documented. (2) is straightforward — change the line-200 call from `(brief.pieceId, result)` to `(brief.pieceId, brief.runId ?? null, result)` matching line 111.
- **Investigation hints:** For (2), run `wrangler d1 execute zeemish --remote --command "SELECT id, run_id, piece_id, beat_name FROM audio_audit_results WHERE run_id IS NOT NULL AND length(run_id) > 36 LIMIT 10"` to check whether any prod rows landed with garbage run_ids. UUID is 36 chars; anything longer is the serialised-object case.
- **Priority:** low. (1) is an existing standing pattern operator has lived with for weeks. (2) is a one-line fix — the only reason it didn't ship in the meter commit is operator's directive on commit hygiene.
