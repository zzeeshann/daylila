...
## [pending operator action] 2026-05-07: Categoriser taxonomy fragmentation — `Knowledge Formation` becoming a dumping ground; new categories spawned with `piece_count=1`

**2026-05-07 — Layer 1+2 of the fix landed (commit `c9938ab`).** Contract rewrite (naming rule: single-word; description rule: domain-level not process-level; foregrounded reuse-test walk-through), prompt JSON example tweaks, recent-headlines + filling-fast density signal threaded into `buildCategoriserPrompt`. **Architectural posture locked:** rules live in the contract; agent reads it; code only persists D1 rows and shapes JSON envelopes. No regex validation in `categoriser.ts`, no retry-message branches firing on code-detected violations, no verifier scripts testing qualitative compliance — same posture as voice / fact-check / audit / interactive contracts. **Layer 3 — one-shot cleanup scripts landed in the same session** at `scripts/categoriser-cleanup-plan.mjs` (Stage A — Claude designs the target taxonomy from prod D1; writes JSON plan; zero D1 writes) and `scripts/categoriser-cleanup-apply.mjs` (Stage B — reads operator-edited JSON, generates `migrations/0039_categoriser_cleanup.sql`; does NOT execute). RUNBOOK section "One-shot taxonomy cleanup" documents the operator workflow. **This entry flips to `[resolved]` once the operator runs Stage A → reviews JSON → runs Stage B → reviews migration SQL → applies migration.** The agent code change (Layer 1+2) is already live on the next deploy; the cleanup is a separate operator-driven step. **Why both name AND description rules**: prod D1 sampling confirmed names AND descriptions both encoded a process-level axis (`Knowledge Formation` description: "How knowledge accumulates through systematic observation" — beautifully written, useless as a filter; admits any science). Domain-level rewrite (`Brain` → "Brain anatomy, neuroscience, cognition...") forces the territory framing that lets pieces be excluded on their merits. **Why headlines AND descriptions**: descriptions alone ossify (frozen at creation); headlines alone don't anchor. Together = description bounds territory, headlines reveal drift, mismatch is a signal. **Watch entries** (post-deploy):
