...
The current loop only ships failed-gate feedback to the Integrator (the prompt scaffolding gates each section behind `if (!result.passed)`). That keeps the prompt small and focused, but it also means the model has no signal about which dimensions are passing — and so its rewrite can inadvertently break them. Closing that gap is queued for Phase 4 Task 09 in `docs/foundation-fix/00-MASTER-PLAN.md` (depends on the data this contract's persistence lands).

For now, the rule the Integrator applies in prose form: **read your own revisions before returning them**. If you changed wording for a structure flag, scan for tribe words. If you changed sentences for a fact-check flag, scan for sentence-length and rhythm.
