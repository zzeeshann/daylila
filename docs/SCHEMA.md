...
**19 tables across 25 migrations.**
...
Indexes: `idx_interactives_slug` (explicit, alongside UNIQUE auto-index), `idx_interactives_source_piece` on `source_piece_id`, `idx_interactives_published_at` on `published_at DESC`. Migration: `0022_interactives.sql` (table) + `0025_interactives_quality_tier.sql` (`quality_tier` column).
...
## Migrations summary (25 migrations, 19 tables)
