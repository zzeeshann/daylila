...
**Updated 2026-05-01 night — Path A.1 corrects the URL harvest source.** Path A's first post-deploy cron piece (Lebanon ceasefire, `6e55caab`) showed `sources: []` despite `searchUsed=1` and ~16 verified specific claims. Diagnosis (verified against Anthropic's web_search docs): citation metadata only attaches to text blocks where Claude EXPLICITLY references a search result; paraphrased notes (the dominant production pattern) get no citations. The URLs DO live in the response — in `web_search_tool_result.content[]` (search-hit array). parseResponse extended to walk that array. Verifier extended 8→9 cases (added Lebanon-shape regression). See DECISIONS 2026-05-01 night "FactChecker Path A.1 — correct the URL harvest source".
...
- **(Phase A — prompt nudge biting)** `daily_audit_claims.sources_json` populates with URLs on ≥3 of the verified claims per piece (Camp Mystic baseline was 0/5). Query: `SELECT claim_index, sources_json FROM daily_audit_claims WHERE piece_id = '<NEW-PIECE-ID>' AND round = (SELECT MAX(round) FROM daily_audit_claims WHERE piece_id = '<NEW-PIECE-ID>') ORDER BY claim_index;`. Even partial improvement (3/5 with at least one URL each) is signal. If 3+ post-deploy cron pieces still ship with all-NULL `sources_json`, escalate per below.
- **(Phase A — aggregate Sources line)** Every Phase-A piece's rendered HTML at `/daily/<date>/<slug>/` shows an aggregate `Sources: domain1 · domain2 · ...` line below the existing meta line, with ≥2 distinct domains. View source or use a DOM query: `Array.from(document.querySelectorAll('p')).find(p => p.textContent?.startsWith('Sources:'))`. Each domain links to its first-occurrence URL; mobile (375px) wraps cleanly with no horizontal overflow. Pre-Phase-H pieces and the Phase-H Camp Mystic piece (legacy string-shape `claimReviews`) gracefully omit the line.
- **(Phase A — JSON-LD back-compat)** ClaimReview JSON-LD block count per piece equals the verified-status claim count for BOTH legacy string-shape pieces (Camp Mystic) and Phase-A object-shape pieces. View source for `<script type="application/ld+json">` blocks containing `"ClaimReview"` or run `curl <URL> | grep -o ClaimReview | wc -l`.
...
- **(Phase A) If `sources_json` remains all-NULL across 3+ post-deploy cron pieces:** the soft prompt nudge isn't enough. Add a worked before/after JSON example to [agents/src/fact-checker-prompt.ts](../agents/src/fact-checker-prompt.ts) SOURCES section showing the populated-sources output shape. Ship as a follow-up commit, retry. If even the worked example doesn't bite, consider a stronger lever — pre-fill the `sources` field with `[]` in a JSON skeleton at the end of the prompt to make the field's existence more salient at generation time.
- **(Phase A) If aggregate Sources line shows >5 domains and looks crowded on mobile:** lower the cap in [src/layouts/LessonLayout.astro](../src/layouts/LessonLayout.astro) `sourceDomains` build from 5 to 3. Single-line edit. The dedup-by-hostname logic already minimises overflow risk in practice — most pieces cite 1-3 outlets at most.
