...
- **`docs/examples/interactive-reference.html`** — hand-written canonical reference. 6.6 KB; passes the validator on all 8 rules. Teaches **chokepoints**. File stays on disk as a human-readable reference; **the Lab Renewal (2026-05-17) removed its injection into the HTML Generator's system prompt** because empirically every HTML lab was unconsciously copying its slider+bars pattern across every concept, producing the homogeneity that fed the 35% essence-fail rate.
- **`agents/src/shared/generated/contracts.ts`** — codegenned from `content/voice-contract.md` + the seven `content/*.md` agent contracts. Was a hand-maintained mirror until 2026-05-03; replaced by `agents/scripts/codegen-contracts.mjs` in Foundation Fix Task 02 Phase A. Used to also export `INTERACTIVE_HTML_REFERENCE` (codegenned from `docs/examples/interactive-reference.html`); that export was removed in the Lab Renewal (2026-05-17) when the Generator prompt stopped injecting the reference.
- **`agents/src/interactive-generator-prompt.ts`** — used to carry a `# Reference example` section embedding the canonical HTML inside `INTERACTIVE_HTML_GENERATOR_PROMPT`'s cached block; removed in the Lab Renewal. The system prompt is now a leaner trust-framing skeleton ("make it beautiful, make it teach, make it immersive — pick the shape that fits THIS concept's mechanism") with the voice contract + interactive contract injections still in place.
