...
- **SAME-EVENT/SAME-CONCEPT prompt (2026-04-27 evening, defense-in-depth):** "Already published recently" block split into two named MUST-skip failure modes — SAME NEWS EVENT (same SCOTUS case, lawsuit, investigation, bill, scandal, person's death, disaster — even at different procedural moments or wire angles) and SAME UNDERLYING CONCEPT (same chokepoint, incentive trap, bias, regulatory mechanic). Three worked examples. "PREFER" replaced with "MUST pick a different candidate". This change shipped after a same-day twin-pieces incident and was intended to be the durable fix; it failed at the first test (see next bullet). It stays in the prompt as the second layer of defence — the headline-overlap filter catches same-event same-headline; this rule still has to catch same-concept different-event (Hormuz + Suez chokepoints share zero substantive headline tokens). See DECISIONS 2026-04-27 (evening, later) "Curator duplicate-pick — SAME-EVENT / SAME-CONCEPT rule".
- **Pre-Curator dedup filter (2026-04-27 architectural fix, the durable layer):** After three rounds of prompt tightening (2026-04-24, 2026-04-26, 2026-04-27 evening) all failed to stop Curator picking the same SCOTUS / cell-location story for the FOURTH time in one day, shifted from prompt-language to data-layer fix. [`agents/src/shared/dedup-headlines.ts`](../agents/src/shared/dedup-headlines.ts) tokenises each candidate's headline + each recent piece's headline, drops stopwords, counts substantive-token overlap. Candidates sharing ≥4 substantive tokens (or ≥3 at ratio ≥0.5) with any recent piece are removed BEFORE Curator sees them. Curator literally cannot pick what it cannot see. Wired in `triggerDailyPiece` between `getRecentDailyPieces` and `curator.curate`. Defensive fallback: if the filter would eat every candidate, pass the original list and log warn. Observer event `Candidates filtered: N of 50 (headline overlap with recent pieces)` surfaces filter activity. Regression harness at `agents/scripts/verify-dedup.mjs` (`pnpm verify-dedup`) — 14 cases, 13 pass + 1 documented borderline. See DECISIONS 2026-04-27 (architectural fix) "Curator duplicate-pick — hard pre-Curator headline-overlap filter" for the full diagnosis + four rejected alternatives (more data, semantic embeddings, pre-Scanner clustering, two-step Curator).
- **File:** `agents/src/curator.ts`
- **Prompt:** `agents/src/curator-prompt.ts`
- **Dedup filter:** `agents/src/shared/dedup-headlines.ts`
...
- **HTML interactive support (Phase 2):** `generate()` runs both quiz + html paths internally with per-type idempotence. The html path is gated by `admin_settings.interactives_html_enabled` (default `'false'`); when on, the Generator runs `runHtmlLoop` as a parallel produce → validate → audit → revise loop with the same 3-round budget. Validator at `agents/src/interactive-validator.ts` runs BEFORE the auditor — 8 rules covering size cap, sandbox compliance, dynamic-code, network calls, etc. Validator max-fail = no commit; auditor max-fail = ship-as-low. Quiz + html share a slug (one URL per piece) — the html path looks up the existing quiz row and inherits its slug; collision resolution is type-scoped via the migration 0026 `UNIQUE(slug, type)` shape.
