...
Migration: `0045_director_health.sql`. Empty at migration time. Rows accumulate one per pipeline run + per audio-retry click; healthy operations live ~30s to ~10 minutes then transition to `completed`. Heartbeat wiring (PR #66, 2026-05-18) updates `last_heartbeat_at` every 30s via setInterval inside `DirectorAgent.startOperationHeartbeat()` — distinguishes healthy long-running operations (heartbeat advances) from genuinely stuck ones (heartbeat doesn't move) for the watchdog scan. Retention rule added in PR #66 (2026-05-18): 30-day window for rows where `status != 'running'`; running rows NEVER pruned (the watchdog at HH:30 owns those by flipping to `orphaned`). Sizing math: 2 pipelines/day + ~5 audio retries/month = ~62 rows/month, ~62 retention-eligible after 30 days. Inside D1 limits indefinitely.
