...
**26 tables across 40 migrations.**
...
Seeded values: `interval_hours = '24'` (preserves current 1-piece/day production cadence until Phase 3 wires the hourly gate); `interactives_html_enabled = 'false'` (migration 0024); `reading_mode = 'scroll'` (migration 0041).

Migrations: `0016_admin_settings.sql` (table + interval_hours seed), `0024_interactives_html_flag.sql` (interactives_html_enabled seed), `0041_reading_mode_setting.sql` (reading_mode seed).
...
## Migrations summary (36 migrations, 26 tables)
...
- `0041_reading_mode_setting.sql` — Beat-by-beat reading mode C4 (2026-05-08). Single `INSERT OR IGNORE` row into `admin_settings` seeding `reading_mode = 'scroll'` so the existing single-scroll layout stays the default after migration apply. Idempotent (matches the 0016 / 0024 / 0025 pattern). Operator flips to `'paginated'` from `/dashboard/admin/settings/` once ready to test in the wild. LessonLayout reads at SSR time via `SELECT value FROM admin_settings WHERE key = 'reading_mode'` and fails open to `'scroll'` on any DB error. Closed-set `['scroll', 'paginated']` enforced at the POST writer in `src/pages/api/dashboard/admin/settings.ts`. After ~7 days of stable paginated traffic, FOLLOWUPS C7 cleanup will drop the flag entirely. Forward-only, additive. Rollback = `DELETE FROM admin_settings WHERE key = 'reading_mode'` (functionally identical to setting back to 'scroll' since the SSR reader fails open). See DECISIONS 2026-05-08 "C4: admin_settings.reading_mode + LessonLayout SSR read + admin toggle".
