...
**Phase 3, sub-task 3.4 — Cost telemetry on the admin interactives page.** Shipped 2026-04-26. Captured `cache_creation_input_tokens` + `cache_read_input_tokens` fields from the Anthropic API response at every Claude call site in InteractiveGenerator + InteractiveAuditor via a new shared `extractUsage()` helper at `agents/src/shared/usage.ts`. Threaded the four counters (input / output / cacheCreate / cacheRead) through both `runQuizLoop` + `runHtmlLoop` accumulators, both artefact result shapes, and the `logInteractiveGeneratorMetered` observer event's metrics + body. New "Cost (month-to-date · MMM YYYY)" section on `/dashboard/admin/interactives/` reads the metered observer events for the current calendar month, parses context JSON in app code, sums all four counters, computes cost using Sonnet 4.5 published rates ($3/M input, $15/M output, cache write at 1.25× input, cache read at 0.1× input). Surface auto-detects when any event in the window lacks cache fields (pre-3.4 events) and renders an italic footnote noting the breakdown is partial for those rows.

`pnpm build` clean. Agents typecheck unchanged at 27 (all pre-existing server.ts SDK-typing). Auth gating verified locally. Post-deploy verification: today's auto-cron runs will produce events with the new cache fields populated; the cost surface should show real numbers within the next 12h slot. See DECISIONS 2026-04-26 "Interactives v3 Phase 3.4 — cost telemetry on the admin interactives page".
...
**Phase 3, sub-task 3.5 — Doc sync.** Update `docs/AGENTS.md` (Phase 3 admin surface — toggle, list view, regenerate, cost telemetry; behaviour notes for InteractiveGenerator + InteractiveAuditor including cache-field capture and where it shows up); `docs/RUNBOOK.md` already has flag-flip + regen-procedure sections — extend with a "where to read MTD cost" pointer to `/dashboard/admin/interactives/`; `docs/INTERACTIVES.md` (the spec doc) gets a Phase 3 admin-surface section. After 3.5: tag `interactives-v3.3-complete`, then proceed to Phase 4 (engagement signals into Learner — `interactive_engagement` table + InteractiveFrame postMessage + Learner reads it).
